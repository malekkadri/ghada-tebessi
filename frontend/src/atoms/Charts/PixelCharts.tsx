...
const PixelCharts: React.FC<PixelChartsProps> = ({ pixels }) => {
    ...
    datasets: [
      {
        ...
        data: [
          pixels.filter(p => p.is_blocked).length,
          pixels.filter(p => !p.is_blocked).length,
        ],
        backgroundColor: [
          'rgba(255, 99, 132, 0.7)',
  ...
  const pieOptions: ChartOptions<'pie'> = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Pixel Status Distribution',
        color: textColor,
        font: {
          size: 16,
